...
7. **Crash-window orphans are accepted, deliberately.** If the process dies between the last `put` and the commit, unreferenced bytes remain on disk. The read path can never reach them (no row references them). Ceiling and upgrade path: add `list(prefix)` to `packages/storage` plus a sweep of keys absent from `page.storageKey`. Not built now — one host, local disk, and a maintenance task is a later concern.
...
- **`storage` gains nothing.** `put`/`delete` suffice; `list` arrives only with the orphan sweep (invariant 7).
